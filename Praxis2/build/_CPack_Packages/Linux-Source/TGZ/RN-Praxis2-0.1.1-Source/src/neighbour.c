...
int peer_is_responsible(uint16_t pred_id, uint16_t peer_id, uint16_t hash_id) {
    ...
    //printf("Peer_Is_Responsible:\nPred_Id: %d, Peer_Id: %d, Hash_Id: %d\n", pred_id, peer_id, hash_id);
    return (hash_id <= peer_id && hash_id > pred_id && pred_id < peer_id) || //138-202-200
            (pred_id > peer_id && hash_id > pred_id); //202 - 10 - 230
